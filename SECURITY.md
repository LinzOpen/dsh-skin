...
The thing worth attacking here is the linter in `css-guard`. A skin is CSS that gets injected
...
这个项目值得攻击的是 `css-guard` 里的检查器。皮肤是被注入进正在运行的界面的 CSS，
而这个项目的全部承诺就是「皮肤碰不到网络」。

**In scope**

- A stylesheet that passes `css-guard validate` but still reaches the network, executes script, or
